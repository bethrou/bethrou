...
const (
	...
	FastestStrategy    PoolStrategy = "fastest"
...
func (p *Pool) Add(peerID peer.ID, addr string) {
	...
	logging.Trace("pool: connection added", "peer", peerID, "addr", addr, "size", len(p.conns))
}
...
func (p *Pool) Remove(peerID peer.ID) {
	...
	for i, conn := range p.conns {
		if conn.PeerID == peerID {
			...
			logging.Trace("pool: connection removed", "peer", peerID, "size", len(p.conns))
			return
		}
	}
}
