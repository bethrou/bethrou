...
// NewClient creates a new libp2p host with the given configuration. priv, if
// non-nil, gives the host a persistent identity (required when the peer ID
// must remain stable across restarts, e.g. once enrolled with a control
// plane); if nil, a fresh ephemeral identity is generated.
func NewClient(keyPath string, priv crypto.PrivKey) (*Client, error) {
	if keyPath == "" {
