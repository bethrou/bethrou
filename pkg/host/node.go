...
type NodeConfig struct {
	...
	ConnectRelay string
	Key          string
	// RelayACL, if set, restricts who may reserve a relay slot or connect
	// through this node's relay (only used when RelayMode is true). If nil,
	// the relay accepts any peer, matching the previous unrestricted
	// behavior.
	RelayACL relayv2.ACLFilter
}
...
func NewNode(cfg NodeConfig) (*Node, error) {
	...
	node := &Node{host: h, acl: cfg.RelayACL}

	logging.Info("Peer ID", "id", h.ID())
	logging.Info("Listening on", "addrs", h.Addrs())
	...
	if cfg.ConnectRelay != "" {
		if err := node.ConnectRelay(ctx, cfg.ConnectRelay); err != nil {
			logging.Warn("Warning: failed to connect to relay", "error", err)
		}
	}
...
func (n *Node) StartRelay() error {
	logging.Info("Starting relay service on this node")

	var opts []relayv2.Option
	if n.acl != nil {
		opts = append(opts, relayv2.WithACL(n.acl))
	}

	relay, err := relayv2.New(n.host, opts...)
	if err != nil {
	...
	logging.Info("Relay service started successfully")
	logging.Info("Relay addresses")
	for _, addr := range n.host.Addrs() {
		logging.Info("address", "addr", fmt.Sprintf("%s/p2p/%s", addr, n.host.ID()))
	}
...
func (n *Node) ConnectRelay(ctx context.Context, relayAddr string) error {
	logging.Info("Connecting to external relay", "relay", relayAddr)
	...
	logging.Info("Connected to external relay successfully")
